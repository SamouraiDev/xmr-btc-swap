    ...
    D: BatchDatabase,
{
    ...
    /// Builds a partially signed transaction from the given recipient address
    /// and amount. Ensures that the address script is at output index `0`
    /// for the created transaction.
    pub async fn send_to_address(
        ...
        psbt.outputs.sort_by(|a, _| {
            if a.witness_script == Some(script.clone()) {
                Ordering::Less
            } else {
                Ordering::Greater
            }
